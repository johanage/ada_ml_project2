...
x, y = np.meshgrid(x, y)
X = np.c_[np.ones(x.ravel().shape), x.ravel(), y.ravel()]
...
# Sklearn
dnn = MLPRegressor(hidden_layer_sizes=(n_neurons,), activation='logistic', solver = method,momentum = 0.9,
...
nn.add_layer(nodes = n_neurons, af = af)# weights = np.random.normal(0,.1,size = (X_train.shape[1], n_hidden_neurons) ) )
...
axs[0].plot(np.arange(len(nn.losses)), nn.losses, label = 'FFNN loss layers 1 ')
...
ax_nn.scatter(X_test[:,1], X_test[:,2], np.sum(nn.predict(X_test), axis=1), c = np.sum(nn.predict(X_test), axis=1),  
...
# save figure
fig.savefig(os.getcwd() + "/plots/prob_b/franke_linreg_ffnn_comparison_eta_%.3f.png"%(eta ), dpi = 150)
fig1.savefig(os.getcwd() + "/plots/prob_b/franke_surface_linreg_ffnn_comparison_eta_%.3f.png"%(eta ), dpi = 150)

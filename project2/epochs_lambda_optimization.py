...
for method in stoch_methods:
    ...
    fig = plt.figure()
    plt.title("Optimization methods regularization $\\eta = %.3f$"%eta)
    for i in range(len(lambdas)):
        # per epoch
        if method in stoch_methods:
            plt.plot(np.arange(epochs)+1, mses[i][:,-1], label="%s $\\lambda = %.4f$"%(labels[method],lambdas[i]))
        if method in det_methods:
            plt.plot(np.arange(max_iter)+1, mses[i], label="%s $\\lambda = %.4f$"%(labels[method],lambdas[i]))
    
    plt.xlabel("Epochs")
    plt.ylabel("MSE")
    plt.legend()
    if method in stoch_methods:
        fig.savefig(os.getcwd() + "/plots/prob_a/epochs_lambdas_mse_%s_eta_10%.2f_gamma_%.2f_beta1_%.2f_beta2_%.2f.png"%(method, np.log10(eta), gamma, beta1, beta2 ), dpi=150)
    if method in det_methods:
        fig.savefig(os.getcwd() + "/plots/prob_a/epochs_lambdas_mse_%s_eta_10%.2f_gamma_%.2f.png"%(method, np.log10(eta), gamma ), dpi=150)

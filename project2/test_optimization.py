...
optimizer = optimizers(X, ynoisy, cost_OLS, eta=1e-1, max_iter = 300)
optimizer(method = 'sgd', epochs = 200,size_mini_batches = 10, **{'lambda' : 1e-3,'scheme': 'linear', 't0' : 1e-1, 't1' : 1e-2})
plt.plot(x,X @ optimizer.theta, label="pred SGD")
plt.legend()

optimizer = optimizers(X, ynoisy, cost_OLS, eta = 1e-1,w_mom = True, verbose=True)
optimizer(method = 'adagrad_sgd', epochs = 200, size_mini_batches = 10, **{'scheme': 'linear', 't0' : 1e-1, 't1' : 1e-2})
...
optimizer(method = 'rms_prop', epochs = 200,size_mini_batches = 10, **{'scheme': 'linear', 't0' : 1e-1, 't1' : 1e-2})
plt.plot(x,X @ optimizer.theta, label="RMS propagation")

optimizer = optimizers(X, ynoisy, cost_OLS, eta=1e-1, max_iter = 300)
optimizer(method='adam', epochs = 200,size_mini_batches = 10, **{'lambda' : 0})
